...
lift2 binaryFunc fa fb = binaryFunc <$> fa <*> fb
...
-- lift3 tFunc fa fb fc = tFunc <$> fa <*> fb <*> fc --this works also
lift3 tFunc fa fb fc = (lift2 tFunc fa fb) <*> fc
...
-- lift4 qFunc fa fb fc fd = qFunc <$> fa <*> fb <*> fc <*> fd --this works also
lift4 qFunc fa fb fc fd = (lift3 qFunc fa fb fc) <*> fd
...
filtering p ls =
  foldRight (\a -> lift2 (\b -> if b then (a:.) else (id)) (p a) ) (pure Nil) ls
  -- same way except explicitly writing 'acc' in lambda funcs.
  -- foldRight (\a acc -> lift2 (\b -> if b then (a:.) else (id)) (p a) acc) (pure Nil) ls
