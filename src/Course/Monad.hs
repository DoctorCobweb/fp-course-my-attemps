...
-- [2,2,3,3]
--
-- apparently Kleisli composition is normal function composition but
-- appropriately bound using bind =<<
(<=<) :: Monad f => (b -> f c) -> (a -> f b) -> a -> f c

(<=<) g h = (=<<) g . h -- how does this work exactly??
-- this is my implementation using lambdas.
-- (<=<) bfc afb = (\x -> (\y -> bfc y) =<< (afb x))
