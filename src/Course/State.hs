...
-- exec state = (\x -> snd ((runState state) x))
...
-- ((),1)
put :: s -> State s ()
put seed = State { runState = (\s -> ((),seed))}
...
instance Functor (State s) where
  ...
  (<$>) f (State lambda) = State (\s -> let (a, s') = lambda s in (f a, s')) 
...
instance Applicative (State s) where
  ...
  (<*>) (State f) (State a)= State (\s -> let (g, t) = f s
                                              (z, u) = a t
                                          in (g z, u)) 
  -- (<*>) funcInState stateA = State (\s -> let f = eval funcInState s
  --                                             s' = exec funcInState s
  --                                             s'' = exec stateA s'
  --                                             a = eval stateA s'
  --                                         in (f a, s''))
...
findM func Nil = return Empty
findM func (h :. t) = (\x -> if x then return (Full h) else findM func t ) =<< func h


------------------------------------------------------------
-- dre :: I GOT UP TO HERE AND GAVE UP. NEED MORE SKILLS FIRST
------------------------------------------------------------
