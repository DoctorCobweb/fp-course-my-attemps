...
seqOptional
  Nil = Full Nil
seqOptional
  (Empty :. xs) = Empty -- need this pattern check as xs may be infinite
seqOptional
  vals = let empties = length $ filter (== Empty) vals -- vals is NOT infinite here
         in if empties == 0
            then Full $ foldRight (\(Full x) acc -> x:.acc) Nil vals
            else Empty
...
-- >>> lengthGT4 infinity
-- True
lengthGT4 :: (Eq a, Num a) =>
  List a
  -> Bool
lengthGT4
  Nil = False
lengthGT4
  vals = if drop 4 vals == Nil
         then False
         else True
...
reverse
  Nil = Nil
reverse
  vals = let step = if even $ length vals  -- gotta handle even/odd lengths differently
                    then (length vals) - 1
                    else length vals
             newFirst = drop step vals
             newLast = take 1 vals
             newSmallerList = drop 1 (take step vals)

         in newFirst ++ reverse newSmallerList ++ newLast
